...
description: "lin2db & db2lin 的计算方法与快速实现"
tags: ["音频", "算法"]
categories: ["技术"]
---
...
-   **指数位 (Exponent Bits)**：8位，表示数字的量级，类似于科学计数法中的 10^x 里的 x。为了能表示正负指数，它采用**偏移表示法 (biased representation)**，即实际指数值需要减去一个固定偏移量（对于单精度浮点数是127）。
...
经过这两步操作，`p.f` 现在的值被强制转换为一个介于 1.0 和 2.0 之间的浮点数（即 \(x^{\prime}\)），而它的**尾数部分保持不变**。原始数字 X 可以被表示为 \(X = X^{\prime} \times 2^{\log_2}\)，其中 \(X^{\prime} \in [1.0, 2.0)\)。

#### 3.3 多项式近似 \(\log_2(x^{\prime})\)
在多项式计算之前，我们已经将原始数字 x 分解成了以下形式：
$$
x = x^{\prime} \times 2^{\log_2}
$$
其中：
...
$$
这里的 `p.f` 在等式右侧代表了 \(x^{\prime}\)。因此，这个多项式是一个**二次多项式** \(P(x^{\prime})\)，它的形式是 \(Ax^{\prime 2} + Bx^{\prime} + C\)，其中 \(A=-1/3\)，\(B=2\)，\(C=-2/3\)。
...
**第二步：展开 \(\log_2(x)\)**
我们将 x 替换为 \(x^{\prime} \times 2^{\log_2}\)：
$$
\log_2(x) = \log_2(x^{\prime} \times 2^{\log_2})
$$
利用对数乘法法则 \(\log_b(A \times B) = \log_b(A) + \log_b(B)\)：
$$
\log_2(x) = \log_2(x^{\prime}) + \log_2(2^{\log_2})
$$
再利用对数幂法则 \(\log_b(A^C) = C \times \log_b(A)\)：
$$
\log_2(x) = \log_2(x^{\prime}) + \log_2 \times \log_2(2)
$$
由于 \(\log_2(2) = 1\)，所以：
$$
\log_2(x) = \log_2(x^{\prime}) + \log_2
$$
...
$$
20 \times \log_{10}(x) = 20 \times \frac{\log_2(x^{\prime}) + \log_2}{\log_2(10)}
$$
这与代码中的最后一行完美匹配：
```c
y=(p.f+log_2)*6.020599913279624f;
...
-   `6.020599913279624f` 这个常数就是 \(\log_2(10) \times 20\) 的精确值。
